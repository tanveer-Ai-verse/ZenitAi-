    ...
    if txt.strip():
        ...
        if errors and st.button("🤖 AI Grammar Fix", key="ai_fix"):
            ...
            with st.spinner("Analyzing..."):
                issues_text = "\n".join(errors)
                ai_resp = groq_ai(f"Correct this text: '{txt}'\nIssues: {issues_text}")
                st.markdown(f'<div class="elite-card">{ai_resp}</div>', unsafe_allow_html=True)
    else:
    ...
    if txt.strip():
        ...
        if st.button("🚀 Ask AI", key="ai_ask"):
            ...
            with st.spinner("Thinking..."):
                response = groq_ai(prompts[mode])
                st.markdown(f'<div class="elite-card">{response}</div>', unsafe_allow_html=True)
    else:
